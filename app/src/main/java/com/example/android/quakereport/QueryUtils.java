...
/**
 * Created by Engr. Temitope on 11/30/2017.
 */
...
public final class QueryUtils {
     ...
     */
    public static ArrayList<quakes> extractquakes() {
        ...
        try {
            ...
            //JSONObject featureOne = new JSONObject();
            //JSONObject jsonProperties = new JSONObject();
            JSONObject jsonfile = new JSONObject(SAMPLE_JSON_RESPONSE);
            JSONArray featuresList =(JSONArray) jsonfile.get("features");
            for (int j=0; j<featuresList.length();j++)
            {   JSONObject featureOne =featuresList.getJSONObject(j);

                JSONObject jsonProperties =featureOne.getJSONObject("properties");
                double magnitude =jsonProperties.getDouble("mag");
                //System.out.println("mag: "+ jsonMag);

                String location = jsonProperties.getString("place");
                //System.out.println("place: "+ jsonPlace);

                Long time = jsonProperties.getLong("time");

                //System.out.println("time: "+ jsonTime);
                quakes earthquake = new quakes(magnitude, location, time );
